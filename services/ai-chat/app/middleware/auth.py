...
async def get_current_user(
    ...
    try:
        ...
        if settings.skip_token_verification:
            logger.warning(
                "⚠️  SKIP_TOKEN_VERIFICATION is enabled - not validating signature!"
            )
            decoded = jwt.decode(
                token,
            ...
            logger.info(
                f"Token decoded (no verification). Claims: {list(decoded.keys())}"
            )
        else:
        ...
        logger.info(
            f"User authenticated: {user.email or user.preferred_username} ({user.id})"
        )
        return user
